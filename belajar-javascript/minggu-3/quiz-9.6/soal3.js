...
function countProfit(shoppers) {
  ...
  for (let i = 0; i < listBarang.length; i++) {
    ...
    for (let ii = 0; ii < shoppers.length; ii++) {

      if (shoppers[ii].product == transactionList[i].product && transactionList[i].leftOver>= shoppers[ii].amount) {
        transactionList[i].shoppers.push(shoppers[ii].name)
        transactionList[i].leftOver -= shoppers[ii].amount
        transactionList[i].totalProfit += shoppers[ii].amount * listBarang[i][1]
      }
