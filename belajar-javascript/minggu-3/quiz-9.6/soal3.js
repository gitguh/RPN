...
function countProfit(shoppers) {
  ...
  if (shoppers.length < 1) {
    ...
    for (let i = 0; i < listBarang.length; i++) {
      ...
      let productPrice = listBarang[i][1]
      let productQty = listBarang[i][2]
      let buyer = []
      let profit = 0

      for (let j = 0; j < shoppers.length; j++) {

        let productDemand = shoppers[j].amount

        if (shoppers[j].product === productName && productQty >= productDemand) {
          buyer.push(shoppers[j].name)
          productQty -= productDemand
          profit += productPrice * productDemand
        }
